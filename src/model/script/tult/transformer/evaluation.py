...
def evaluate_detailed(model, window_gen, scaler):
    ...
    csv_path = 'metrik_performa_tult.csv'
    komplit_metrik = {
        'Model': 'TULT TRANSFORMER',
        'Z_MAE': round(mae, 4),
        'Z_RMSE': round(rmse, 4),
        'Z_R2': round(r2, 4),
...
def plot_results(labels, preds, r2, history=None, metrics=None):
    ...
    fig.suptitle('Dashboard Evaluasi Model TULT TRANSFORMER', fontsize=18, weight='bold')
    ...
    plt.savefig('dashboard_evaluasi_tult_transformer.png', dpi=300)
    print("\n[+] Dashboard Evaluasi berhasil disimpan ke 'dashboard_evaluasi_tult_transformer.png'")
